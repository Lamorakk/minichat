...
function connect(event) {
    ...
    if (nickname && email) {
        if (adminPassword === "admin") {
            nickname = 'admin'; // Admin special access
        }
        ...
        stompClient.connect({}, function () {
            ...
            if (nickname === 'admin') {
                ...
                stompClient.send("/app/user.addUser", {}, JSON.stringify({nickName: nickname, email: email, status: 'ONLINE'}));
            }
        }, onError);
    }
}
...
// JavaScript adjustments would focus mainly on ensuring users can only send to and receive from admin.
function sendMessage(event) {
    const messageContent = messageInput.value.trim();
    if (messageContent && stompClient) {
        const chatMessage = {
            senderId: nickname,
            recipientId: 'admin', // Admin ID, assumed constant
